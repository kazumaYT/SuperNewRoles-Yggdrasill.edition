...
namespace SuperNewRoles
{
    ...
    class AllRoleSetClass
    {
        ...
        public static float GetPlayerCount(RoleId RoleDate)
        {
            switch (RoleDate)
            {
                ...
                case (RoleId.Fox):
                    ...
                    return CustomOption.CustomOptions.DoubleKillerPlayerCount.getFloat();
                    //プレイヤーカウント
            }
            return 1;
        }
        ...
        if (!(CustomOption.CustomOptions.DoubleKillerOption.getString().Replace("0%", "") == ""))
            {
                int OptionDate = int.Parse(CustomOption.CustomOptions.DoubleKillerOption.getString().Replace("0%", ""));
                RoleId ThisRoleId = RoleId.DoubleKiller;
                if (OptionDate == 10)
                {
                    Impoonepar.Add(ThisRoleId);
                }
